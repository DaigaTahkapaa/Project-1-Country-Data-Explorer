...
async function load() {
  ...
  try {
    ...
    const res = await fetch(
      "https://restcountries.com/v3.1/all?fields=name,cca2,region,capital,population"
    );
    ...
    if (statusMessage)
      statusMessage.textContent = "Failed to load countries. Please try again.";
...
function card(c) {
  ...
  // Use CSS class so layout is controlled from styles.css
  ...
  // Use CSS class so hover/active rules apply consistently
  btn.className = "btn-fav";
  // If already favourited, mark the button active so it gets the favourited color
  ...
  btn.addEventListener("click", () => {
    ...
    saveFav();
    // update immediate visual state (class) so button color changes instantly,
    // then re-render to rebuild cards correctly
    btn.classList.toggle("active", !!fav[c.code]);
    btn.textContent = !!fav[c.code] ? "Remove favourite" : "Add favourite";
    render();
  });
  // Add button to actions container
  actions.appendChild(btn);

  // If the country is favourited, show a small standalone heart icon (no badge).
  // Include an sr-only label for screen readers while keeping the visual icon minimal.
  if (liked) {
    ...
    `;
    // Add heart to actions container
